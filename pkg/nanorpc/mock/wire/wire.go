...
// mustBeValid panics when a required field is nil, wrapping [core.ErrInvalid]
// so a recovering caller can match the family. It names the first missing
// field so the misconfiguration is attributable at the [New] call site.
func (cfg Config[Out, In]) mustBeValid() {
	switch {
	case cfg.Conn == nil:
		core.PanicWrap(core.ErrInvalid, "wire.New: nil Conn")
	case cfg.Split == nil:
		core.PanicWrap(core.ErrInvalid, "wire.New: nil Split")
	case cfg.Encode == nil:
		core.PanicWrap(core.ErrInvalid, "wire.New: nil Encode")
	case cfg.Decode == nil:
		core.PanicWrap(core.ErrInvalid, "wire.New: nil Decode")
	default:
...
// until the connection ends or the peer is closed. It panics with a
// [core.ErrInvalid]-wrapped payload when a required field — Conn, Split,
// Encode or Decode — is nil, surfacing the misconfiguration synchronously
// at the call site rather than as a later async panic in the reader or Send.
