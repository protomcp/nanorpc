...
// TestServer_Close_cleansUpAfterWorkgroupCancelled pins the cleanup fix:
// when something has already cancelled the workgroup (e.g. an accept fault),
// Close must still close the listener and release every tracked connection
// rather than returning early. Gating cleanup on winning wg.Cancel — as the
// previous code did — leaked them.
func TestServer_Close_cleansUpAfterWorkgroupCancelled(t *testing.T) {
	srv := New(t)

	// Track a connection so Close has something to release.
	a, b := net.Pipe()
	defer func() { _ = a.Close() }()
	c := newConn(t, b)
	core.AssertMustTrue(t, srv.track(c), "track conn")

	// Simulate the accept loop faulting first: the workgroup is already
	// cancelled, so the Cancel inside Close returns false.
	srv.wg.Cancel(nil)
	...
	srv.mu.Lock()
	closed := srv.closed
	openLen := len(srv.open)
	srv.mu.Unlock()
	core.AssertTrue(t, closed, "server marked closed")
	core.AssertEqual(t, 0, openLen, "tracked conns released")
}
