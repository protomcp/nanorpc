...
// both [core.ErrInvalid] — the base the package's sentinels wrap — and
// [os.ErrInvalid] (== fs.ErrInvalid) anywhere in the chain, since the two
// are distinct identities despite sharing the "invalid argument" text.
func IsInvalid(err error) bool {
	return core.IsErrorFn(checkIsInvalid, err)
}

func checkIsInvalid(err error) bool {
	return err == core.ErrInvalid || err == os.ErrInvalid
}
