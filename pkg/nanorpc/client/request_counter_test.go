...
func testBasicCounter(t *testing.T) {
	...
	// Test first ID
	id1 := counter.Next()
	core.AssertTrue(t, id1 > 0, "First ID should be positive")

	// Test second ID (should be different)
	id2 := counter.Next()
	core.AssertTrue(t, id2 > 0, "Second ID should be positive")
	core.AssertNotEqual(t, id1, id2, "IDs should be different")

	// Test sequential nature
	id3 := counter.Next()
	core.AssertEqual(t, id2+1, id3, "Expected sequential increment")
}
...
func testNilReceiver(t *testing.T) {
	...
	id := counter.Next()
	core.AssertTrue(t, id > 0, "Nil receiver should return positive random ID")
}
...
func testSkipZero(t *testing.T) {
	...
	for i := range 1000 {
		id := counter.Next()
		core.AssertNotEqual(t, int32(0), id, "Counter should never return 0 at iteration %d", i)
		core.AssertTrue(t, id > 0, "Counter should never return negative at iteration %d", i)
	}
}
...
func testConcurrency(t *testing.T) {
	...
	for i := range results {
		ids, _ := core.AssertTypeIs[[]int32](t, results[i], "Failed to get result %d as []int32", i)
		for j, id := range ids {
			core.AssertTrue(t, id > 0, "ID at goroutine %d, index %d should be positive", i, j)
			core.AssertFalse(t, seen[id], "Duplicate ID found: %d", id)
			seen[id] = true
		}
	}
}
...
func testWraparound(t *testing.T) {
	...
	id1 := counter.Next()
	core.AssertEqual(t, int32(2147483647), id1, "Expected MaxInt32")

	id2 := counter.Next()
	core.AssertEqual(t, int32(1), id2, "Expected wraparound to 1 (skipping 0)")

	id3 := counter.Next()
	core.AssertEqual(t, int32(2), id3, "Expected 2 after wraparound")
}
...
func testRandomRequestIDUniqueness(t *testing.T) {
	...
	core.AssertTrue(t, len(seen) > minExpected, "Expected at least %d unique IDs, got %d", minExpected, len(seen))
}
