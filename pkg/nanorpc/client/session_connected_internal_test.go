...
// newConnectedSession wires a spawned [Session] to a mock [server.Conn] over
// an in-memory net.Pipe and attaches it to the [Client], so the public
...
func newConnectedSession(t *testing.T) (*Client, *server.Conn) {
	t.Helper()
	...
	// Registered before server.Serve so it runs after the mock server's own
	// cleanup (t.Cleanup is LIFO): the server closes its pipe end first,
	// then the client session unwinds.
	t.Cleanup(func() {
		_ = ss.Close()
		_ = cs.Wait()
	})

	return c, server.Serve(t, srvConn)
}
